...
so we didn't send them an invitation. We did send invitations to everyone else!",
    ...
    def test_invite_with_welcome_bot_custom_message_from_admin(self) -> None:
        self.login("iago")
        invitee = self.nonreg_email("alice")
    ...
    def test_invite_without_welcome_bot_custom_message_from_admin(self) -> None:
        self.login("iago")
        invitee = self.nonreg_email("alice")
    ...
    def test_invite_with_welcome_bot_custom_message_from_member(self) -> None:
        ...
        invitee = self.nonreg_email("bob")
        result = self.invite(invitee, [], welcome_message_custom_text="custom message text")
        self.assert_json_error(result, "Must be an organization administrator")

    def test_invite_without_welcome_bot_custom_message_from_member(self) -> None:
        # If the `welcome_message_custom_text` is configured for the realm.
        self.login("hamlet")
        realm = get_realm("zulip")
        invitee = self.nonreg_email("alice")
...
class MultiuseInviteTest(ZulipTestCase):
    ...
    def test_invite_with_welcome_bot_custom_message_from_member(self) -> None:
        ...
        # Non-admin users are not allowed to send customized welcome messages.
        result = self.client_post(
            "/json/invites/multiuse", {"welcome_message_custom_text": "custom message text"}
        )
        self.assert_json_error(result, "Must be an organization administrator")
