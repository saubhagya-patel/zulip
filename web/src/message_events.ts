...
import {message_edit_history_visibility_values} from "./settings_config.ts";
    ...
    for (const event of events) {
        ...
        if (anchor_message !== undefined) {
            ...
            if (event.orig_content !== undefined) {
                if (
                    realm.realm_message_edit_history_visibility ===
                    message_edit_history_visibility_values.always.code
                ) {
        ...
        if (!topic_edited && !stream_changed) {
            ...
            for (const moved_message of event_messages) {
                if (
                    realm.realm_message_edit_history_visibility !==
                    message_edit_history_visibility_values.never.code
                ) {
